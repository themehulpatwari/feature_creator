...
# Get columns from all_metrics that are NOT in base
...
# Prepare all_metrics for merge - keep merge keys + unique columns
merge_cols = ['query_id', 'user_id', 'task_id'] + unique_metrics_cols
all_metrics_subset = all_metrics_df[merge_cols]

# Rename base columns to match all_metrics (lowercase)
base_df_renamed = base_df.rename(columns={'query_ID': 'query_id'})

# Merge on query_id, user_id, and task_id using inner join (intersection)
